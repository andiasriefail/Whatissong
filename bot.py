...
def convert_ogg_opus_to_mp3(input_path: str) -> str:
    ...
    try:
        ...
        logger.warning(f"Konversi libopus gagal: {e}")
        try:
            subprocess.run(
            ...
            logger.warning(f"ffmpeg gagal total: {e2}")
            return input_path
...
def recognize_with_audd(file_path: str = None, url: str = None):
    ...
    for i, api_key in enumerate(AUDD_API_KEYS):
        try:
            ...
            logger.error(f"AudD key #{i+1} exception: {e}")
            continue
    return False
...
def get_lyrics(artist: str, title: str):
    try:
        ...
        logger.error(f"Error get lyrics: {e}")
    return None
...
async def handle_audio(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        converted_path = convert_ogg_opus_to_mp3(tmp_path) if ext == ".ogg" else tmp_path
